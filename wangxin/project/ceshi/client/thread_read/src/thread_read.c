            ...
            case SENDTO:
                {
                    ...
                    write(sockfd,&file_len,sizeof(file_len));
                    char * data = (char *)malloc(file_len + 1);
                    memset(data,0,file_len + 1);
                    block_read(fd,data,file_len);
                    block_write(sockfd,data,file_len);
                    printf("文件已发送,等待对方处理！\n\n");
                    break;
                }
            case SEND:
                {
                    ...
                    printf("%s 向你发送文件,是否接收？(yes/no)",msg1.toname);
                    gets(buf);
                    write(sockfd,&buf,10);
                    if(strcmp(buf,"yes") != 0)
                    {
                    ...
                    read(sockfd,&file_len,sizeof(file_len));

                    char * data = (char *)malloc(file_len + 1);
                    memset(data,0,file_len + 1);
                    block_read(sockfd,data,file_len);
                    printf("请输入你要另存文件的路径:");
                    int fd;
                    while(1)
                    {
